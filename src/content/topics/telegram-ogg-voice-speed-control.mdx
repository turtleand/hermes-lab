---
title: "Fix Telegram Voice Speed Control in Hermes with OGG Opus"
module: "Module 0: Setup & Voice"
subtopic: "telegram-ogg-voice-speed-control"
summary: "Why Hermes Telegram TTS replies need OGG Opus voice files for waveform and playback speed controls, and how ffmpeg fixes MP3 fallback."
status: "done"
articles: []
---
...
1. `hermes doctor` warns when Telegram voice mode is enabled but `ffmpeg` is missing.
2. The setup wizard explains that Telegram voice bubbles require OGG Opus.
3. The TTS pipeline logs a clear warning when it falls back to MP3 attachments.
4. Documentation links the symptom, missing speed controls, to the actual cause, wrong audio format.
...
- [Hermes Agent TTS docs](https://hermes-agent.nousresearch.com/docs/user-guide/features/tts/)
